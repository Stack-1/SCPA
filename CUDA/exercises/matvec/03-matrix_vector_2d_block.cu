...
const dim3 BLOCK_DIM(1,16);
// Calculate the dimension of the grid of blocks (2D).
const dim3 GRID_DIM(16,1);
  ...
  */
  ...
  int col = threadIdx.x;
  int thread_count_row = gridDim.x * blockDim.y;
  int thread_count_col = gridDim.y * blockDim.x;
